...
import { getAliasTargetName, isCompoundValue, getCompoundReferences } from "utils/tokens/utils"
...
function tokenTypeToFigmaType($type: JsonTokenType): VariableResolvedDataType | null {
	switch ($type) {
		...
		case "boolean":
			return "BOOLEAN"
		case "string":
		case "fontFamily":  // Treat as string for now
			return "STRING"
		case "fontWeight":  // Convert to number/float
			return "FLOAT"
		// These composite types are not supported by Figma Variables
		case "shadow":
		case "typography":
		case "strokeStyle":
		case "border":
...
export async function importTokens(files: Record<string, JsonTokenDocument>, manifest?: JsonManifest): Promise<OperationResult[]> {
	...
	do {
		...
		for (const update of queuedUpdates) {
			...
			// Check if this is a compound value with multiple references
			if (isCompoundValue(update.token.$value)) {
				// For now, skip compound values as Figma Variables don't support them
				results.push({
					result: "info",
					text: `Skipping ${update.figmaName} because compound values like "${update.token.$value}" aren't supported by Figma Variables.`,
				})
				continue
			}
